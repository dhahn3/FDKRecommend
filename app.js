...
function suppressBLSPairing(out, plan){
  ...
  // Stations (3-digit) of any selected A units
  const aStations = new Set(out.filter(x=>x.cap==='A').map(x=> (trailing(x.st) || trailing(x.unit) || x.st) ));
  ...
  return out.filter(x => !(x.cap==='BLS' && suppressStations.has(trailing(x.st) || trailing(x.unit) || x.st)));
}
...
async function boot(){
  if(typeof DATA==='undefined'){ banner().className='warn'; banner().textContent='DATA missing'; return; }
