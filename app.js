...
function isAvailable(unit){
  const st = (CURRENT_STATUS[unit] || 'AQ').toUpperCase();
...
function buildCandidatesByOrder(eszOrder, capsNeeded, used){
  ...
  for(const [idx, stRaw] of eszOrder.entries()){
    ...
    for(const u of units){
      ...
      if(!isAvailable(u)) continue; // respect AQ/PA
...
// Core recommender (unchanged logic, but skip PA units everywhere)
function recommendUnitsRunCard_Additive(esz){
  ...
  for(const g of groups){
    for(let i=0;i<g.qty;i++){
      // HM33 rule (skip in 50xx ESZ) but respect AQ/PA
      ...
      // OR groups with preferences: (A OR MP) prefer A; (BR OR E) prefer BR  — all respecting AQ/PA
...
function resetStatusToDefaults(){
...
function renderStatusModal(){
  ...
  for(const st of eszOrderAllStations){
    ...
    for(const u of units){
      const caps = (DATA.UNIT_CAPS[u]||[]).join(', ');
      const stv = (CURRENT_STATUS[u] || 'AQ').toUpperCase();
      const cls = stv==='AQ' ? 'status-AQ' : 'status-PA';
      html += `<tr><td>${u}</td><td>${caps}</td><td>
  ...
  cont.querySelectorAll('select[data-unit]').forEach(sel => {
    sel.addEventListener('change', e => {
      ...
      e.target.className = (val==='AQ') ? 'status-AQ' : 'status-PA';
      saveStatus();
    });
  });
}
...
async function main(){
  ...
  try{
    ...
    console.error(e);
    DATA = {ESZ_ORDER:{}, STATION_UNITS:{}, UNIT_CAPS:{}, UNIT_STATUS:{}, PLAN_STRUCT:{}};
  }

  // Load status (defaults + any saved overrides)
  loadStatus();
  ...
  $('#btnResetStatus').addEventListener('click', resetStatusToDefaults);

  // Send dispatch => set all currently recommended units to CALL
