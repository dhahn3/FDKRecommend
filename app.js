...
async function fetchESZForPoint(lat,lng){
  ...
  const url = `${ESZ_URL}/query?`+encodeParams({
    ...
    outFields:'*', returnGeometry:true, outSR:4326, f:'json'
      ...
      const recDiv = $('#rec');
      recDiv.innerHTML = '<div class="warn">Recommendation failed: ' + (err.message||err) + '</div>';
      const dbg = window.__lastDebug || {}; dbg.error = String(err && err.stack || err);
...
function buildCandidatesByOrder(eszOrder, capsNeeded, used){
  ...
  // DD-if-closer rule: if any E was selected in the response plan,
  // and a DD-capable unit is earlier in the run-card order than the closest E,
  // send that DD in addition to the closest E.
  (function(){
    // DD-if-closer rule (enhanced): if any E was selected by the plan, and a DD-capable unit
    // appears earlier in the run-card order than the closest selected E, add that DD as well.
    const ePicks = responseSelections.filter(sel => sel.roleCap === 'E');
    if(!ePicks.length) return;
    const closestERank = Math.min(...ePicks.map(s => (Number.isFinite(s.rank)?s.rank:999999)));
    const ddCandsAll = buildCandidatesByOrder(order, ['DD'], new Set()); // don't exclude by 'used' for the scan
    const ddEarlier = ddCandsAll.filter(c => c.rank < closestERank && isAvailable(c.unit)); ddEarlier.sort((a,b)=>a.rank-b.rank);
    const chosen = ddEarlier.length ? ddEarlier[0] : null;
    // Debug surface
    if(!window.__lastDebug) window.__lastDebug = {};
    window.__lastDebug.ddCheck = {closestERank, ddCandidates: ddCandsAll.map(c=>({u:c.unit,st:c.st,rank:c.rank}))};
    if(chosen && !used.has(chosen.unit)){
      used.add(chosen.unit);
      results.push({unit: chosen.unit, roleCap: 'DD', from: 'IFC', st: chosen.st, rank: chosen.rank});
    }
  })();
...
function recommendUnitsRunCard_Additive(esz){
  ...
  // Built-in If Closer additions from plan
...
async function main(){
  ...
  $('#btnRecommend').addEventListener('click', ()=>{ /* REC-TRY-CATCH */ try {
    const esz=$('#eszInfo').textContent.trim();
    const recDiv=$('#rec');
