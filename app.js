...
function recommendUnitsRunCard_Additive(esz){
  ...
  // DDK-if-closer rule (multi + backfill):
  // If the plan includes K and any DDK units appear earlier than the closest selected K,
  // add *all* such DDKs. Then ensure total K >= number of DDKs added (add K backfill if needed).
  (function(){
    ...
    const ddkEarlier = ddkAll.filter(c => c.rank < closestKRank && isAvailable(c.unit) && !used.has(c.unit));
