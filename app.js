...
let DATA=null, MAP, MARKER, CURRENT_ESZ=null, HILITE=null;
...
function stationUnits(st){
  ...
  if(d){ keys.add(d); keys.add(`LCF/${d}`); keys.add(`MCF/${d}`); }
  const out = [];
...
async function setESZFromPoint(lat, lon){
  const url = new URL(`${ESZ_URL}/query`);
  const p = url.searchParams;
  p.set('f','json');
  p.set('geometry', JSON.stringify({x: lon, y: lat, spatialReference:{wkid:4326}}));
  ...
  if(!js.features || !js.features.length){ banner().className='warn'; banner().textContent='No ESZ here'; return; }
  const f = js.features[0];
  const esz = f.attributes?.FIRE_ESZ || f.attributes?.ESZ || '';
  setESZ(String(esz));
  try{
    const rings = f.geometry?.rings?.[0];
    if(Array.isArray(rings)){
      const latlngs = rings.map(([x,y])=>[y,x]);
      if(HILITE){ MAP.removeLayer(HILITE); }
      HILITE = L.polygon(latlngs, {color:'#3b82f6', weight:2, fillOpacity:0.15}).addTo(MAP);
    }
  }catch{}
}
...
function runRecommend(){
  ...
  const res2 = suppressBLSPairing(res);
...
async function boot(){
  ...
  try{
    ...
    MAP.on('click',async e=>{ if(MARKER) MARKER.remove(); MARKER=L.marker(e.latlng).addTo(MAP); await setESZFromPoint(e.latlng.lat, e.latlng.lng); });
  ...
  B('btnLocate').onclick=async ()=>{
    const mode=B('searchMode').value; const q0=B('q').value.trim();
    if(!q0) return;
    try{
      const url = new URL('https://nominatim.openstreetmap.org/search');
      url.searchParams.set('format','json'); url.searchParams.set('bounded','1'); url.searchParams.set('limit','1');
      url.searchParams.set('viewbox','-77.85,39.75,-77.00,39.15'); // Frederick County
      let q=q0;
      if(mode==='intersection'){
        const parts=q0.split('&').map(s=>s.trim());
        if(parts.length>=2){ q = `${parts[0]} & ${parts[1]}, Frederick County, MD`; }
      }else{
        q = `${q0}, Frederick County, MD`;
      }
      url.searchParams.set('q', q);
      const r = await fetch(url, {headers:{'Accept':'application/json'}});
      const js = await r.json();
      if(!Array.isArray(js) || !js.length) throw new Error('not found');
      const {lat, lon} = js[0];
      if(MARKER) MARKER.remove();
      MARKER=L.marker([+lat,+lon]).addTo(MAP);
      MAP.setView([+lat,+lon], 13);
      await setESZFromPoint(+lat,+lon);
      banner().className='ok'; banner().textContent='Ready';
