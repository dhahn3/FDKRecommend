...
function stationUnits(st){
  ...
  const out = [];
  for(const k of keys){
    const arr = (DATA?.STATION_UNITS?.[k]) || [];
...
function suppressBLSPairing(out, plan){
  ...
  const aStations = new Set(out.filter(x=>x.cap==='A').map(x=> trailing(x.st)||trailing(x.unit)||String(x.st||'').toUpperCase()));
  const suppress = new Set();
  if(aStations.has('924')) suppress.add('911');
  if(aStations.has('930')) suppress.add('910');
  if(!suppress.size) return out;

  return out.filter(x => !(x.cap==='BLS' && suppress.has(trailing(x.st)||trailing(x.unit)||String(x.st||'').toUpperCase())));
}
...
function runRecommend(){
  ...
  const res2 = suppressBLSPairing(res, plan);
...
async function boot(){
  if(typeof window.DATA==='undefined'){ banner().className='warn'; banner().textContent='DATA missing'; return; }
  DATA=window.DATA;
