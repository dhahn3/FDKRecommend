...
function openStatus(){
  ...
  for(const st of stations){
    ...
    // Staffing override toggle for affected stations
    if(CROSS_RULES[st]){
      const ov=document.createElement('label'); ov.className='toggle small';
      const cb=document.createElement('input'); cb.type='checkbox'; cb.checked=STAFF_OVERRIDE.has(st);
      cb.onchange=()=>{ if(cb.checked) STAFF_OVERRIDE.add(st); else STAFF_OVERRIDE.delete(st); };
      ov.appendChild(cb); ov.appendChild(document.createTextNode(' Ignore staffing rules for this station'));
      grid.appendChild(ov);
    }
...
async function boot(){
  if(window.DATA){ DATA = window.DATA; } else { const r=await fetch('data.json?v=55'); DATA=await r.json(); }
