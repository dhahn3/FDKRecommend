...
function recommendUnitsRunCard_Additive(esz){
  ...
  // DDK-if-closer rule: if the plan calls for K and a DDK unit is earlier than the closest selected K,
  // add that DDK in addition to the K.
  (function(){
    ...
    const closestKRank = Math.min(...kChosen.map(s => Number.isFinite(s.rank)?s.rank:999999));
    const ddkAll = buildCandidatesByOrder(order, ['DDK'], null); // scan all available DDK
    const ddkEarlier = ddkAll.filter(c => c.rank < closestKRank && !used.has(c.unit));
    ddkEarlier.sort((a,b)=>a.rank-b.rank);
    if(ddkEarlier.length){
      const c = ddkEarlier[0];
      used.add(c.unit);
  ...
  if (planCallsForE){
    ...
    if (eChosen.length){
      const closestERank = Math.min(...eChosen.map(s => Number.isFinite(s.rank)?s.rank:999999));
      const ddAll = buildCandidatesByOrder(order, ['DD'], null); // scan all available DD (ignore used for scan)
      const ddEarlier = ddAll.filter(c => c.rank < closestERank && !used.has(c.unit));
