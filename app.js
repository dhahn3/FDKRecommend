...
async function main(){
  try{
    ...
    catch(e){ const w=$('#loadWarn'); w.style.display='block'; w.textContent='Failed to load data.json — make sure index.html, app.js, styles.css, data.json and .nojekyll are published together.'; DATA={ESZ_ORDER:{}, STATION_UNITS:{}, UNIT_CAPS:{}, UNIT_STATUS:{}, PLAN_STRUCT:{}, NON_HYDRANT_ESZ:[]}; }
    ...
    $('#btnRec').addEventListener('click', ()=>{
      try{
        ...
        function groupRows(arr){ const g={}; for(const r of arr){ (g[r.roleCap]=g[r.roleCap]||[]).push(r.unit); } return Object.entries(g).sort((a,b)=>a[0].localeCompare(b[0])); }
