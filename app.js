...
async function main(){
  ...
  $('#btnRecommend').addEventListener('click', ()=>{
    ...
    // Build map cap -> units (all rows combined: RESP + IFC)
    const groups = {};
    for (const r of rows) {
      const cap = r.roleCap || 'OTHER';
      (groups[cap] = groups[cap] || []).push(r.unit);
    }

    // Order capability rows by the RESPONSE PLAN order,
    // then append IF-CLOSER capability rows in their first-seen order,
    // then any remaining caps.
    ...
    // 1) RESPONSE order strictly by plan.groups sequence.
    const planRespOrder = [];
    for (const g of (plan?.groups || [])) {
      const capsNorm = (g.caps || []).map(norm);
      // find the first RESP pick whose roleCap is one of this group's caps
      const hit = respRows.find(r => capsNorm.includes(norm(r.roleCap)));
      if (hit && !planRespOrder.includes(hit.roleCap)) {
        planRespOrder.push(hit.roleCap);
      }
    }

    // 2) IF-CLOSER caps after, preserving first-seen order
    const ifcCapsOrder = [];
    for (const r of ifcRows) {
      if (!planRespOrder.includes(r.roleCap) && !ifcCapsOrder.includes(r.roleCap)) {
        ifcCapsOrder.push(r.roleCap);
      }
    }

    // 3) Any remaining caps that appeared (unlikely), append
    const dynamicOrder = [...planRespOrder, ...ifcCapsOrder];
    for (const cap of Object.keys(groups)) {
      if (!dynamicOrder.includes(cap)) dynamicOrder.push(cap);
    }

    // Render vertically per capability row
