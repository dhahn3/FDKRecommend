...
function runRecommend(){
  ...
  // Handle required groups: array of objects like {qty, caps:[...]}
  for(const grp of (plan.groups||[])){
    ...
    for(const c of cands){
      if(out.some(x=>x.unit===c.unit)) continue;
      out.push(c); used.add(c.unit);
      if(out.filter(x=>wantCaps.includes(x.cap)).length >= qty) break;
    }
    // If still short, try again skipping capability preference (fallback to any of group's caps later in order)
    let have = out.filter(x=>wantCaps.includes(x.cap)).length;
    if(have < qty){
      const more = buildCandidatesByOrder(order, wantCaps, used);
      for(const c of more){
        if(out.some(x=>x.unit===c.unit)) continue;
        out.push(c); used.add(c.unit);
        have = out.filter(x=>wantCaps.includes(x.cap)).length;
        if(have >= qty) break;
      }
    }
  }

  // ifCloser is an array of caps (strings)
  for(const cap of (plan.ifCloser||[])){
    const p = pickForCap(cap, order, used);
    if(p){ out.push(p); used.add(p.unit); }
  }

  // Non-hydrant overrides
  ...
  maybeAddDDK(out, order, used);
  enforceHM33(out, CURRENT_ESZ, used);

  const filtered = suppressBLSPairing(out);
...
function renderRecommendation(arr){
  ...
  const by=new Map();
  for(const it of arr){
    const g=by.get(it.cap)||[]; g.push(`${it.unit} (${it.st})`); by.set(it.cap,g);
  }
...
async function boot(){
  ...
  try{
    ...
    // Frederick County basemap
    L.esri.tiledMapLayer({
      url:'https://fcgis.frederickcountymd.gov/server/rest/services/Basemap/Basemap/MapServer'
    }).addTo(MAP);
