...
function applyEarlierSubFor(selected, order, used, plan, cap, subCap){
  ...
  if(planCaps.has('E')) addEarliestEarlierSubExtra(selected, order, used, plan, 'E', 'DD');
  if(planCaps.has('K')) addEarliestEarlierSubExtra(selected, order, used, plan, 'K', 'DDK');
  

function addEarliestEarlierSubExtra(selected, order, used, plan, primaryCap, subCap){
  ...
  // Compare subs against the *latest* selected primary rank so that a sub
  // qualifies if it's earlier than ANY of the chosen primaries (not just the first).
  const latestPrimRank = Math.max(...chosenPrim.map(x => x.rank));

  // Find the single earliest subCap strictly earlier than the latest primary
  const subs = buildCandidatesByOrder(order, [subCap], used).filter(x => x.rank < latestPrimRank);
  if (!subs.length) return;
  const earliestSub = subs.sort((a, b) => a.rank - b.rank)[0];
  if (!selected.some(x => x.unit === earliestSub.unit)) {
    selected.push(earliestSub);
    used.add(earliestSub.unit);
  }
}
