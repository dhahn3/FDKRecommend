...
window.addEventListener('error', (e)=>{
  const w=document.getElementById('loadWarn');
  if(w){ w.style.display='block'; w.textContent = 'JavaScript error: ' + (e.message || 'see console'); }
});

// If a promise rejects (fetch, etc.), show it too
window.addEventListener('unhandledrejection', (e)=>{
  const w=document.getElementById('loadWarn');
  if(w){ w.style.display='block'; w.textContent = 'Unhandled promise rejection: ' + (e.reason && e.reason.message ? e.reason.message : e.reason); }
});
...
// Inject default incidents if PLAN_STRUCT is empty
function ensureFallbackPlans(){
  try{
    if(!DATA || !DATA.PLAN_STRUCT || Object.keys(DATA.PLAN_STRUCT).length===0){
      ...
      const w=document.getElementById('loadWarn');
      if(w){ w.style.display='block'; w.textContent='Using FALLBACK incident list (data.json has no PLAN_STRUCT)'; }
    }
  }catch(e){}
}
...
async function boot(){
  const w=document.getElementById('loadWarn');
  if(w){ w.style.display='block'; w.textContent='Loading…'; }
  ...
  try{
    ...
    if(!resp.ok){ 
      if(w){ w.style.display='block'; w.textContent='Warning: data.json not found ('+resp.status+'). Using empty dataset.'; }
      DATA = {ESZ_ORDER:{}, PLAN_STRUCT:{}, NON_HYDRANT_ESZ:[], UNIT_CAPS:{}, STATION_UNITS:{}, BC_UNITS:[], UNIT_STATUS:{}}; ensureFallbackPlans();
    } else {
      DATA = await resp.json(); ensureFallbackPlans();
    }
    ...
    try{
      ...
      if(w){ w.style.display='block'; w.textContent='Map init failed: '+(err.message||err); }
    }
...
try{ boot().then(()=>{ const w=document.getElementById('loadWarn'); if(w){ w.style.display='block'; w.textContent='Ready'; w.classList.remove('warn'); w.classList.add('ok'); } }).catch(e=>{ const w=document.getElementById('loadWarn'); if(w){ w.style.display='block'; w.textContent='Startup error: '+(e.message||e); }}); }catch(e){ const w=document.getElementById('loadWarn'); if(w){ w.style.display='block'; w.textContent='Fatal boot error: '+(e.message||e); }}
