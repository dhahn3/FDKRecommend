...
/* FDK Recommend – multi-file build v43 */
...
async function boot(){
  try{
    // Load data.json explicitly so we keep multi-file separation
    const resp = await fetch('data.json?v=v43');
    if(!resp.ok){ throw new Error('Failed to load data.json ('+resp.status+')'); }
    DATA = await resp.json();

    // Merge persisted status/caps
