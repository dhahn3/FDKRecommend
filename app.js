...
function buildCandidatesByOrder(eszOrder, capsNeeded, used){
  ...
  (function(){
    ...
    const ddCands = buildCandidatesByOrder(order, ['DD'], used);
    const closer = ddCands.find(c => c.rank < closestERank);
    if(closer){
      used.add(closer.unit);
      results.push({unit: closer.unit, roleCap: 'DD', from: 'IFC', st: closer.st, rank: closer.rank});
    }
  })();
