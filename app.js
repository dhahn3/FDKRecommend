...
let CURRENT_STATUS   = {};   // unit -> AQ/CALL/PA
let CURRENT_CAPS_OVR = {};   // unit -> [tokens]

function setMsg(el, text, cls='small'){ el.className = cls; el.textContent = text; }
...
function showESZ(feature){
  ...
  $('#order').textContent = order.length ? ('Station order: ' + order.map(v=>normalizeStationId(v)).join(' → ')) : 'No station order on file.';
...
function recommendUnitsRunCard_Additive(esz){
  ...
  const entry = (DATA?.PLAN_STRUCT||{})[key]; if(!entry) return [];
  const is50  = String(esz||'').startsWith('50');

  const selPlan = decidePlanForESZ(esz, entry);
  ...
  $('#vocab').style.display = unknown.size ? 'block' : 'none';
  $('#vocab').textContent = unknown.size ? ('Unknown capability tokens (no unit advertises: ' + Array.from(unknown).join(', ')+')') : '';
    ...
    for(let i=0;i<g.qty;i++){
      ...
      if(g.caps.map(normCap).includes('BC')){
        ...
        for(const [idx,stRaw] of order.entries()){
          const s=normalizeStationId(stRaw);
  ...
  // If-closer (earlier than earliest RESP pick)
  ...
  // DD additive vs E (earlier than worst E) + backfill E
  ...
  if(planCallsForE){
    ...
    if(eChosen.length){
      ...
      const unique = []; const seen = new Set(); // avoid duplicates
  ...
  // DDK additive vs K (earlier than worst K) + backfill K
  ...
  // BLS special cases if ambulance is from 924 or 930
...
// Status & caps storage
function loadStatus(){ try{ CURRENT_STATUS = {...(DATA?.UNIT_STATUS||{}), ...(JSON.parse(localStorage.getItem(STATUS_KEY)||'{}'))}; } catch(_){ CURRENT_STATUS = {...(DATA?.UNIT_STATUS||{})}; }
function saveStatus(){ localStorage.setItem(STATUS_KEY, JSON.stringify(CURRENT_STATUS)); }
function loadCaps(){ try{ CURRENT_CAPS_OVR = JSON.parse(localStorage.getItem(CAPS_OVR_KEY)||'{}'); } catch(_){ CURRENT_CAPS_OVR = {}; } }
function saveCaps(){ localStorage.setItem(CAPS_OVR_KEY, JSON.stringify(CURRENT_CAPS_OVR||{})); }

// Modal: status & capability editor
function openStatusModal(){
  ...
  for(const st of stations){
    const units = (DATA.STATION_UNITS[st]||[]).slice().sort();
    ...
    for(const u of units){
      ...
      capsBox.addEventListener('change', ()=>{
        ...
        CURRENT_CAPS_OVR[u] = toks; saveCaps();
      });
      ...
      ['AQ','CALL','PA'].forEach(s=>{
        ...
        r.addEventListener('change', ()=>{ CURRENT_STATUS[u]=s; saveStatus(); });
  ...
  $('#btnResetCaps').onclick = ()=>{ CURRENT_CAPS_OVR = {}; saveCaps(); openStatusModal(); };
...
async function main(){
  ...
  try{ const r=await fetch('data.json?v='+Date.now()); DATA=await r.json(); }
  catch(e){
    $('#loadWarn').style.display='block'; $('#loadWarn').textContent='Failed to load data.json — deploy all files together.';
  ...
  function decidePlanForIncident(esz, key){
    const entry=(DATA?.PLAN_STRUCT||{})[key];
    if(!entry) return null;
    const isNH = (DATA?.NON_HYDRANT_ESZ||[]).includes(String(esz).padStart(4,'0'));
    const hasNH = (entry?.groupsNH||[]).length>0;
    return {
      groups:  (isNH && hasNH) ? (entry.groupsNH||[]) : (entry.groupsStd||[]),
      ifCloser:(isNH && hasNH) ? (entry.ifCloserNH||{}) : (entry.ifCloserStd||{}),
      flavor:  (isNH && hasNH) ? 'NON-HYDRAN' : 'STANDARD',
      isNH
    };
  }
  ...
  function updatePlanView(){
    ...
    const key=$('#incident').value;
    const entry=(DATA?.PLAN_STRUCT||{})[key];
    if(!entry){ $('#planTxt').textContent=''; $('#ifcTxt').textContent=''; $('#flavor').textContent=''; return; }
    const selPlan = decidePlanForIncident(esz, key);
    const groups=selPlan.groups||[]; const ifc=selPlan.ifCloser||{};
    $('#planTxt').textContent = 'Plan: ' + groups.map(g=> (g.qty>1? g.qty+'× ' : '') + g.caps.join(' OR ')).join(', ');
    ...
    $('#flavor').innerHTML = '<span class="small">'+badge+'</span>' + (selPlan.isNH && selPlan.flavor!=='NON-HYDRAN' ? ' — (no specific NON-HYDRAN plan for this incident)' : '');
  }
  ...
  $('#btnRec').addEventListener('click', ()=>{
    try{
      ...
      function renderGroup(title, arr){
        ...
        const line = document.createElement('div'); line.className='units';
        for(const u of arr){
          const span=document.createElement('span'); span.className='pill rec'; span.textContent=u; line.appendChild(span);
        }
      ...
      div.innerHTML='';
      const byRoleResp = groupRows(respRows);
      const byRoleIfc  = groupRows(ifcRows);
      for(const [cap, units] of byRoleResp){ div.appendChild(renderGroup(cap+':', units)); }
      if(byRoleIfc.length){
        const sep=document.createElement('div'); sep.className='small'; sep.style.marginTop='8px'; sep.textContent='(If closer additions):'; div.appendChild(sep);
        for(const [cap, units] of byRoleIfc){ div.appendChild(renderGroup(cap+':', units)); }
      }
      ...
      $('#rec').innerHTML = '<div class="warn">Recommendation failed: '+(e.message||e)+'</div>';
    }
  });
  ...
  $('#mutualAid')?.addEventListener('change', ()=>{ /* re-run */ $('#btnRec').click(); });
}
