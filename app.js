...
// Core recommender (run card + additive IfCloser; special HM rules)
function recommendUnitsRunCard_Additive(esz){
  ...
  const responseSelections = []; // keep details for baseline
  ...
  for(const g of groups){
    for(let i=0;i<g.qty;i++){
      ...
      if (g.caps.map(x => String(x).toUpperCase()).includes('HM') && i===0 && !isESZ50) {
        ...
        // Fill remaining HM count from run-card cands (skip HM33 to avoid duplicate)
        ...
        // Add an extra HM if one exists earlier than HM33 in run-card
      ...
      // BC group: pick first BC by run-card order
      ...
      // Normal capability OR group with special tie-breakers for (A OR MP) and (BR OR E)
  ...
  // Baseline for If-Closer (ambulance anchor)
  ...
  // 2) IF-CLOSER add-ons: only if earlier than baseline
  ...
  // 3) BLS EXCEPTIONS: if ambulance from 924 or 930, drop BLS from 911 or 910 respectively (only roleCap==='BLS')
...
async function main(){
  try{
    const res = await fetch('data.json?v=' + Math.random());
    DATA = await res.json();
  }catch(e){
  ...
  map.on('click', async (e)=>{
    ...
    catch(err){ setMsg($('#geocodeMsg'), err.message); }
  });
  ...
  $('#btnSearch').addEventListener('click', async ()=>{
    ...
    try{
      ...
      showESZ(eszFeat);
    }catch(e){ setMsg($('#geocodeMsg'), e.message||'Search failed'); $('#eszInfo').textContent='—'; $('#eszOrder').textContent=''; eszLayer.clearLayers(); }
  });
  ...
  $('#btnRecommend').addEventListener('click', ()=>{
    ...
    for (const r of respRows) {
      (groupsResp[r.roleCap] = groupsResp[r.roleCap] || []).push(r.unit);
    }
    // Group IFC by cap
    const groupsIfc = {};
    for (const r of ifcRows) {
      (groupsIfc[r.roleCap] = groupsIfc[r.roleCap] || []).push(r.unit);
    }

    // Determine plan-ordered capability rows
    const incidentKey = $('#incidentSelect').value;
    const plan = DATA.PLAN_STRUCT[incidentKey];
    ...
    // Required counts (by chosen cap for OR groups)
    ...
    // RESP rows with placeholders up to required counts
    ...
    if(dbg){
      const out=[];
      out.push(`ESZ: ${dbg.esz}  Incident: ${dbg.incident}`);
