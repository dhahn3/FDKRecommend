...
function suppressBLSPairing(out){
  // Apply only when BLS is an ifCloser need (not a required group)
  const incSel = (typeof B==='function' && B('incident')) ? B('incident').value : null;
  const plan = incSel && DATA?.PLAN_STRUCT?.[incSel];
  let blsInGroups=false, blsIfCloser=false;
  if(plan){
    ...
    blsIfCloser = (plan.ifCloser||[]).map(normCap).includes('BLS');
  }
...
async function boot(){
  const r=await fetch('data.json?v=55'); DATA=await r.json();
